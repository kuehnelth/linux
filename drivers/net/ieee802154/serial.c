...
static int
ieee802154_serial_set_laddr(struct zb_device *zbdev, __le64 addr)
{
	int ret = 0;
	uint8_t len = 0;
	...
	//if (mutex_lock_interruptible(&zbdev->mutex)) { pr_debug("%s no lock :(\n", __func__);
	//	return -EINTR; }
	...
	buf[len++] = (uint8_t)(addr >> 56);
	buf[len++] = (uint8_t)(addr >> 48);
	buf[len++] = (uint8_t)(addr >> 40);
	buf[len++] = (uint8_t)(addr >> 32);
	buf[len++] = (uint8_t)(addr >> 24);
	buf[len++] = (uint8_t)(addr >> 16);
	buf[len++] = (uint8_t)(addr >> 8);
	buf[len++] = (uint8_t)(addr >> 0);
...
static int
ieee802154_serial_get_laddr(struct ieee802154_dev *dev, __le64 addr)
{
	struct zb_device *zbdev;
	int ret = 0;
	...
	if (wait_event_interruptible_timeout(zbdev->wq,
		...
		addr = 0;
		addr += (__le64) zbdev->data[0] << 56;
		addr += (__le64) zbdev->data[1] << 48;
		addr += (__le64) zbdev->data[2] << 40;
		addr += (__le64) zbdev->data[3] << 32;
		addr += (__le64) zbdev->data[4] << 24;
		addr += (__le64) zbdev->data[5] << 16;
		addr += (__le64) zbdev->data[6] << 8;
		addr += (__le64) zbdev->data[7] << 0;
		if (zbdev->status != STATUS_SUCCESS)
			ret = -EBUSY;
	} else
		ret = -ETIMEDOUT;
					    ...
					    unsigned long changed)
{
	...
	if (mutex_lock_interruptible(&zbdev->mutex)) { BUG();
		return -EINTR;}

	switch (changed)
	{
		case IEEE802515_AFILT_SADDR_CHANGED:
			ret = send_cmd3(zbdev, CMD_SET_SHORT_ADDRESS, filt->short_addr & 0xff, filt->short_addr >> 8);
			break;
		case IEEE802515_AFILT_PANID_CHANGED:
			ret = send_cmd3(zbdev, CMD_SET_PAN_ID, filt->pan_id & 0xff, filt->pan_id >> 8);
			break;
